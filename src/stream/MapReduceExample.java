...
public class MapReduceExample {
    ...
    public static void main(String[] args) {
        List<User> users = Arrays.asList(new User("jack", 40),
                new User("john", 32), new User("jill", 47),
                new User("mike", 28));
    ...
    private static double countAvarageWithLambda(List<User> users) {
        return users.stream().filter(u -> u.getAge() > 0)
                .mapToDouble(u -> u.getAge()).average().getAsDouble();
    }

    private static double countSumWithReduce(List<User> users) {
        return users
                .stream().reduce(new User(), (User n1,
                        User n2) -> new User("", n1.getAge() + n2.getAge()))
                .getAge();
    }
}
